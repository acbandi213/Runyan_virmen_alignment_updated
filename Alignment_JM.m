%% provide all inputs
info.mousename = 'IG-6-1L';%;
info.mouse = info.mousename;
info.date = '2024-08-03'; %;
info.server = '/Volumes/Runyan4'; %/Volumes/Runyan5
runyan5 = "/Volumes/Runyan5:";
runyan4 = '/Volumes/Runyan4';
data_base = 'JMIG-6-1L_240803';%;
info.experimenter_name = 'Jordyn';
...
info.sync_base_path = ['/Volumes/Runyan4/Jordyn/Jordyn Data/RawData/' info.mousename '/wavesurfer/' info.date '/'];
info.virmen_base = ['/Volumes/Runyan4/Jordyn/Jordyn Data/RawData/' info.mousename '/virmen/' data_base];
info.imaging_base_path=['/Volumes/Runyan4/Jordyn/Jordyn Data/RawData/' info.mousename '/' info.date '/'];
info.save_path = ['/Volumes/Runyan4/Jordyn/Jordyn Data/ProcessedData/' info.mousename '/' info.date '/'];
info.processed_path = ['/Volumes/Runyan4/Jordyn/Jordyn Data/ProcessedData/' info.mousename '/' info.date '/spikes/'];

info.is_stim_dataset = 1; 
% give data inputs!
info.galvo_channel = 5;
info.virmen_channel = 6;
info.vr_sync_string = '2024'; %string within VR files to look for so it doesn't read other unnecessary files
...
load(strcat('/Volumes/Runyan5/Connie/condition_per_speaker.mat'));
...
%% load imaging data
load(strcat(info.processed_path,'Fall_red_dff.mat'));
load(strcat(info.processed_path,'Fall_red_deconvsig.mat'));
dff_red = dff; %added by CB
deconv_red = deconvsig; %added by CB

load(strcat(info.processed_path,'dff.mat'));
load(strcat(info.processed_path,'deconvsig.mat'));
dff_non = dff; %added by CB
deconv_non = deconvsig; %added by CB
% dff = zeros(10,15e4);
% deconv = dff;
%% load virmen data
...
%mkdir(strcat(info.server,'/Akhil', '/ProcessedData/',num2str(info.mouse),'/',num2str(info.date)))
cd(strcat(info.server,'/Jordyn/Jordyn Data/', '/ProcessedData/',num2str(info.mouse),'/',num2str(info.date)));
...
% 
% % sound_info.sync_sampling_rate = alignment_info(1).sync_sampling_rate;
% % sound_info.distance_between_sounds = 1*sound_info.sync_sampling_rate ;%min distance between sounds in digidata units in task is ~4 seconds between reward sound and start of trial- (passive at 10k was about 45000)
% % sound_info.distance_within_sounds = [1940, 4140]; %for task should be 200
% % sound_info.sound_duration = [260];%[0.99*sound_info.sync_sampling_rate,1.1*sound_info.sync_sampling_rate];
% % sound_info.correct = .250; %correct_trial_ITI_length in seconds
% % sound_info.incorrect = .40; %incorrect_trial_ITI_length in seconds
% % sound_info.smoothing_factor = 15; %almost always 15 sometimes 20
% 
% sound_info.unique_detection_threshold = [];%list specific file and threshold wanted [file#1,threshold1; file#2,threshold2]
% sound_info.detection_threshold = .9; %0.9;%for 1k (0.45)between 0.4 and 0.5 (0.5 gets rid of more noise) - for some 10k 0.8 (one file #8 in HA10-1L\2023-03-24)
...
% 
% %use this one (adjusts gaps to make them smaller)
% [file_estimated_trial_info,file_matching_trials,sound_condition_array] = match_trialsperfile_pulse_maze(digidata_its, trial_info,sound_condition_array,task_info,data); %changes to ITI gap distance 
% 
% 
...
[virmen_it,trial_its,sound_condition_array] = get_virmen_iterations_and_times_digidata_positive_peaks(info.sync_base_path,info.virmen_channel,info.vr_sync_string,[],data,[]);
...
%for now we are using the shift code to get the correct IT iterations
%in the future I want to modify previous iteration code so
%virmen_it/trial_its are calculated only once
...
imaging = align_virmen_data_AB(dff,deconvsig,virmen_it,alignment_info,data,dataCell,[],[],[]);
imaging_stim = align_virmen_data_AB(dff_red,deconv_red,virmen_it,alignment_info,data,dataCell,[],[],[]);
...
save('imaging','imaging');%,'-v7.3');
%save('imaging_stim','imaging_stim');%,'-v7.3');
%save('vr_sound_frames',"vr_sound_frames"); %used in passive
