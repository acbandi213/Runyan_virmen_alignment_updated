...
for vr_trial = 1:length(dataCell.dataCell)-1%1:length(dataCell.dataCell)-1 % virmen trials within each acquisition (ex: VR_stim01)
    ...
    [x,y] = min(abs(data.data(1,:) - dataCell.dataCell{1,vr_trial}.time.start));   %%the first virmen iteration during the current trial based on the start time from virmen (+1 so that it very first value is not ITI)
     y = y+1;
    % x is the min value, y is the index where this value occurs
    [x2,y2] = min(abs(data.data(1,:) - dataCell.dataCell{1,vr_trial}.time.stop));   %%%the last virmen iteration during the current trial
     y2 = y2+1;
    start_time_next_trial = dataCell.dataCell{1,vr_trial+1}.time.start;
    
    start_it = y; %index of first min value
    end_it = y2; %index of the last min value
    iti_start_it = y2+1; % start of the ITI is one unit after the end of the trial
    [xx,yy] = min(abs(data.data(1,:) - start_time_next_trial)); %ITI ends at the start of the next trial
    iti_end_it = yy;
...
%                 this_pure_tone = reward_info(vr_trial,2):reward_info(vr_trial,3); %onset to offset frames of pure tone
            ...
            % check if ITI frames are after gap!
            %ones before NANs of reward period!
%             if isnan(this_ITI(find(this_ITI == 1,1,'first')+1))
%                 figure(55);clf;plot(this_ITI);
%                 error('Error. n\Check ITI iteration location');
%             end
